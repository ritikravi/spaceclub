        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-10">
          {divisions.map((d) => (
            <div key={d.name} className="glass glass-hover rounded-2xl p-5 cursor-pointer">
              ...
              <h3 className="text-gray-900 font-semibold mb-1">{d.name}</h3>
              <p className="text-gray-400 text-xs leading-relaxed">{d.desc}</p>
            </div>
          ))}
        </div>
        <div className="text-center">
          <Link href="/divisions" className="inline-flex items-center gap-2 text-orange-500 hover:text-orange-600 font-semibold text-sm transition-colors">
