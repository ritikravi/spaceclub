        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-10">
          {divisions.map((d) => (
            <div key={d.name} className="glass glass-hover rounded-2xl p-5 cursor-pointer">
              ...
              <h3 className="font-semibold mb-1" style={{ color: "var(--text)" }}>{d.name}</h3>
              <p className="text-xs leading-relaxed" style={{ color: "var(--text-faint)" }}>{d.desc}</p>
            </div>
          ))}
        </div>
        <div className="text-center">
          <Link href="/divisions" className="inline-flex items-center gap-2 text-blue-400 hover:text-blue-300 font-medium text-sm transition-colors">
