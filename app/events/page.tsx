...
const events = [
  ...
  { title: "NASA Space Apps Hackathon Prep", type: "Hackathon", status: "upcoming", date: "Feb 5, 2025", time: "9:00 AM – 5:00 PM", location: "Auditorium, LPU", capacity: "60 seats", description: "Team formation, problem statement analysis, mock pitches, and review from mentors. Full-day prep for NASA Space Apps.", border: "border-blue-400", badge: "text-blue-700 bg-blue-50" },
    ...
    <div className="min-h-screen bg-white pt-20">
      <section className="bg-blue-700 py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <span className="inline-block px-3 py-1 text-xs font-semibold text-blue-200 bg-white/10 rounded-full mb-4">Events</span>
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-6">Events & Activities</h1>
          <p className="text-blue-100 text-lg max-w-xl mx-auto">Workshops, observations, hackathons, visits, and webinars — all year round.</p>
        </div>
      </section>
        ...
        <div className="flex items-center gap-2 mb-10">
          {(["all", "upcoming", "past"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all capitalize ${filter === f ? "bg-blue-700 text-white shadow-sm" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}>
              {f}
            </button>
          ))}
        </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                ...
                {ev.status === "upcoming" && (
                  <div className="shrink-0">
                    <button className="px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white text-sm font-semibold rounded-xl transition-all shadow-sm">
                      Register
                    </button>
                  </div>
                )}
