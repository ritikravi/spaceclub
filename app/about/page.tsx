...
const team = [
  { name: "Dr. Rajesh Kumar", role: "Faculty Coordinator", dept: "Aerospace Dept." },
...
const achievements = [
  { icon: Trophy, value: "Top 40", label: "NASA Space Apps — Global Shortlist", color: "text-yellow-400 bg-yellow-400/10" },
  { icon: BookOpen, value: "12", label: "Research Papers Published", color: "text-pink-400 bg-pink-400/10" },
  { icon: Rocket, value: "3", label: "CanSat Missions Completed", color: "text-orange-400 bg-orange-400/10" },
  { icon: Users, value: "500+", label: "Active Student Members", color: "text-blue-400 bg-blue-400/10" },
  { icon: Building2, value: "5+", label: "ISRO Visits Organised", color: "text-green-400 bg-green-400/10" },
  { icon: Trophy, value: "SIH 2023", label: "Smart India Hackathon Winners", color: "text-purple-400 bg-purple-400/10" },
];
...
export default function AboutPage() {
  return (
    <div className="min-h-screen bg-[#050a14] pt-24">
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-6">
            We Are a{" "}
            <span className="gradient-text">Space Innovation Hub</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="glass rounded-2xl p-8">
            ...
            <p className="text-slate-400 leading-relaxed">
              To cultivate a strong ecosystem of space technology, astronomy, satellite engineering, robotics, AI, and aerospace innovation among students — giving every student a clear path from curiosity to contribution.
            </p>
          </div>
          <div className="glass rounded-2xl p-8">
            ...
            <p className="text-slate-400 leading-relaxed">
              To build India's most active student-led space innovation community — a launchpad for students aiming for ISRO, IN-SPACe, IITs, international research labs, and private space companies.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* Achievements */}
      <section id="achievements" className="py-16 bg-[#060c18]">
            ...
            {achievements.map((a) => (
              <div key={a.label} className="glass glass-hover rounded-2xl p-6 flex items-center gap-5">
                ...
                <div>
                  ...
                  <div className="text-slate-400 text-sm">{a.label}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {team.map((m) => (
            <div key={m.name} className="glass glass-hover rounded-2xl p-6 text-center">
              <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg mx-auto mb-4">
                {m.name.split(" ").map(w => w[0]).join("").slice(0,2)}
              </div>
              <div className="text-white font-semibold">{m.name}</div>
              <div className="text-blue-400 text-sm mt-1">{m.role}</div>
              <div className="text-slate-500 text-xs mt-1">{m.dept}</div>
            </div>
          ))}
        </div>
      </section>
