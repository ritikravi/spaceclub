    ...
    <div className="min-h-screen bg-white pt-20">
      <section className="bg-orange-500 py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <span className="inline-block px-3 py-1 text-xs font-semibold text-orange-200 bg-white/20 rounded-full mb-4">Membership</span>
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-6">Join SpaceClub LPU</h1>
          <p className="text-orange-100 text-lg max-w-xl mx-auto">Start as a Cosmos Member. Work your way up through involvement and contribution.</p>
        </div>
      </section>
      ...
      <section className="py-16 max-w-5xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          {tiers.map((t) => (
            <div key={t.title} className={`glass rounded-2xl p-7 ${t.border} relative ${t.popular ? "ring-2 ring-orange-400 shadow-lg" : ""}`}>
              {t.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-orange-500 text-white text-xs font-bold rounded-full">Most Popular</div>}
            ...
            <form onSubmit={handleSubmit} className="glass rounded-2xl p-8 space-y-5 border-t-4 border-orange-400">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <select id="year" required value={form.year} onChange={e => setForm({...form, year: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-800 text-sm focus:outline-none focus:border-orange-400 transition-all">
              ...
              <div>
                ...
                <select id="division" required value={form.division} onChange={e => setForm({...form, division: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-800 text-sm focus:outline-none focus:border-orange-400 transition-all">
              ...
              <button type="submit" disabled={loading} className="w-full py-3.5 bg-orange-500 hover:bg-orange-600 disabled:opacity-60 text-white font-bold rounded-xl transition-all text-sm shadow-sm">
